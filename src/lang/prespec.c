...
//  where it it.i halt emit print prind namespace typename
//
...
//
//  def func(>> int) #macro {
//      `defer prine("defer inside macro")
//      if `a < `b {
...
let calc = (int a, &int float) { } // (int a) 表示没有返回值 (int a, return int float) 表示有返回值，且返回值不能为空
let calc = (int a, &int float (x y)) { }
