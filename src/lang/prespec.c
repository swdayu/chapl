...
//
// al bl cl dl el fl gl hl il jl kl ol pl ql rl sl tl ul vl wl xl yl zl
// ac bc cc dc ec fc gc hc ic jc kc oc pc qc rc sc tc uc vc wc xc yc zc
// an ar as at au ax be bi bl by cc df dr dt du
// em en el er es ex fe fr gh gl go gr he hi
// id if im in is it io iv ja je ke kr la lf
// ma me ml mr ms mu my na nb nl no nt nu ny
// ob od of oh om on or os ot ou ow ox pa pb pe pf ph pi pl pm po pr ps pu py
// ra re rf ri rl ro rs rv sa se sh sl so to tr tu ul un up ur us ut ve we xi zu
//
...
//  7.  单个文件不能定义名称前缀，导入添加的名字前缀不能声明为公开可见，添加一个前缀后，当前文件不能再使用该前缀定义新的名称
//  8.  还可以批量重命名去掉前缀，但是可能需要做一些额外的限制，比如 log 与数学函数重名，去掉前缀的名字会与标准库名字重名
//  9.  能够去掉前缀的前提，被导入的名字在定义时需要加上 may strip 限定，并在文件中定义前缀名称 def ^prefix_
//  10. 或者直接定义可选的名字 def prh_array_push|array_push([3]int self, int elem) { ... } // 默认使用长名字，当手动剥去前缀时可以使用短名字
...
//  3.  import "std.array" 的时候，实际是导入一个代码包 array，其中的符号必须通过 array::push 进行访问
//  4.  库集合，例如 std，是通过命令行参数或配置文件，如果没有提供使用默认库路径的配置文件，便在编译时传入编译器
//          import "std.array/index" // 在库 std 所定义的目录中查找名为 array 的子目录，标准库可以省略库名称前缀，并使用 <> 形式导入
//          import "vendor.raylib"
//  5.  如果不指定库名称前缀，在代码文件所在的当前文件开始查找
//          import "utils" // 相对于当前文件所在目录查找名为 utils 的子目录
//          import "utils/test" // 相对于当前文件所在目录查找名为 utils 的子目录，以及 utils 下的子目录
//          import "../helpers" // 在上一级目录中查找名为 helpers 的子目录
...
//      栈中。
//
//      别再告诉我成员函数很好，因为你可以输入 obj. 然后在 IDE 中输入点后获得过程列表。
//      你显然也可以用扁平函数做到这一点。请别再告诉我这个了，谢谢。例如数组字符串操作
//      可以以 array_push string_push 等名称，而操作概念化编译器可以移除前缀来识别，
//      或者显式定义概念。
//  9.  每个函数都有一个隐藏的当前环境的指针参数
//
// 函数的名称，以及是否重载。为了编译器的简单性，仅支持显式重载，并且在对应的命名空间
// 中，即使是显式的重载名称，都不能有名字的重叠。另外，也支持操作符的重载，例如 __equal__
// __less__ __add__ __sub__ __mul__ __div__ __mod__ 等显式的重载名称，这些名称都必须是
// 显式的重载名称。
//
// 显式函数重载，为了显式性和简洁性，隐式过程重载会使作用域系统复杂化。在 C++ 中，你不
// 能在过程内部嵌套过程，因此所有过程查找都在全局作用域进行。在 Odin 中，过程可以嵌套
// 在过程内部，因此，在隐式重载的情况下，确定应该使用哪个过程是复杂的。显式重载有许多
// 优点：
//  1.  明确什么被重载了
//  2.  如果需要，能够引用特定的过程
//  3.  清楚实体名称属于哪个作用域
//  4.  如果需要，能够特化参数多态过程，这些过程具有相同的参数但有不同的约束（参见 where 子句）
//
//      def to_string(overloaded) {
//          int_to_string
//          float_to_string
//      }
//
...
// where 子句可能有用的一些情况如下：
//
//  1.  参数的正确性检查，可以分为静态检查 static 和动态检查 where
//      def simple_sanity_check([2]int x) where len(x) > 1 && typeof(x) == [2]int {
//          prinf(x)
//      }
//  2.  过程的参数多态性检查
...
//          prinf(#procedure, "was called with the parameter", x)
//          return true
//      }
//      def bar([$N]int x, &bool) where N > 0 && N <= 2 {
//          prinf(#procedure, "was called with the parameter", x)
//          return false
//      }
//      def baz(overloaded) {
...
//      assert(ok_y == false)
//  4.  记录类型的参数多态参数限制
//      def Foo $(anytype T, int N) where T::type_is_integer && N > 2 {
//          [N]T x
//          [N-2]T y
//      }
//      use T i32
...
//  关于指针
//
//      严格别名优化（Strict-Aliasing Optimization）是 C/C++ 编译器的一种优化策略，基于
//      C/C++ 标准中的严格别名规则（Strict Aliasing Rule）。C/C++ 标准规定：不同类型的指
//      针不能指向同一块内存。即，通过 int* 和 float* 访问同一个地址是未定义行为（Undefined
//      Behavior, UB）。编译器据此假设：不同类型的指针永远不会重叠，从而进行激进优化。
//
// Rust's macro system is significantly more structured than C's, using
// token-stream manipulation rather than raw text substitution. Rust macros can
// be powerful, but procedural macros in particular are widely considered
// complex and difficult to write correctly.
//
// In Jai, metaprogramming is designed to feel like ordinary programming. Code
// can be represented and manipulated as strings, then inserted at compile time
// with #insert. Functions can be generated through string manipulation
// routines. Macros are syntactically indistinguishable from regular function
// calls. The intention is that metaprogramming in Jai should not feel like a
// separate, intimidating system — it should feel like writing normal code that
// happens to run at compile time.
//
// One area where Rust's macro system goes further is arbitrary syntax:
// procedural macros can process token streams flexibly enough that developers
// have embedded small domain-specific languages inside Rust. Jai's out-of-order
// compilation model makes this kind of arbitrary syntax extension incompatible
// with its design, so Jai does not support it.
//
// Rust and Jai both support hygienic Lisp-like macros that execute at compile
// time, , enabling metaprogramming.
//
// LIST 的宏实现：
//
// 宏的实现：
...
//
//  def add(@x) #macro
//  ''' @x + 2 + x // 第二个 x 是当前环境中的一个名称
//
// https://github.com/Jai-Community/Jai-Community-Library/wiki/Getting-Started#macros
//
// 实现卫生宏（hygienic macros），卫生宏不会导致任何意外的标识符捕获，卫生宏仅在显式
// 允许时才能修改变量。与 C 语言中宏完全任意不同，卫生宏更受控，编译器支持更好，并具有
// 更好的类型检查。宏类似于内联函数，编译器会将代码与宏功能内联。任何带有反引号的内容都
// 是宏之外的外部作用域中的内容。在这种语言中，宏的工作方式类似于 Lisp 中的卫生宏：局
// 部变量在局部可用，如果宏引用外部作用域中的内容，需要使用反引号 ` 标记该变量。变量不
// 是唯一可以用反引号 "`" 标记的代码片段。你还可以反引号返回值和 defer 语句。你不能反
// 引号 continue、break 或 remove 语句。反引号返回值意味着宏从外部作用域返回。在此示
// 例中，宏中的反引号 return 从外部函数返回一个字符串值。
//
//  def func(void) #macro {
//      let a = "local variable inside macro" // 宏局部变量，不会污染外部作用域
//      `a += 10 // 操作外部作用域中的变量
//  }
//
...
//
//  def func(>> int) #macro {
//      `defer prinf("defer inside macro")
//      if `a < `b {
...
//  }
//
// 宏可以接受 anycode 作为参数，并可以在宏内部进行扩展将代码插入到宏体中。与普通函数
// 一样，可以从宏返回值。通过宏参数传递内联汇编寄存器，可以通过参数 regname 传递寄存
// 器名称。
//
//  def func(anycode c) #macro {
//      ''' c
//      ''' c
//      ''' c // 将代码 c 插入到宏中三次
//  }
...
//  }
//
// 宏可以嵌套。你可以在另一个宏中调用宏。有一个宏限制，意味着你可以生成的宏调用次数有限
// 制。如果你递归调用宏（例如，创建一个 fibonacci 宏来递归调用 fibonacci），这会导致
// 编译器错误，提示你达到了宏限制。宏限制默认为 1000。
//
//  def nested_macro(void) #macro {
//      prinf("nested macro")
//  }
//
//  def func(void) #macro {
...
//  @test 是一个测试函数，执行 ycc test 命令时执行的函数
//  @cold 是一个极少调用的函数
...
//  resert(expr)
//  devel { stmt ... }
//  devel print print_e print_d print_i
//  devel prinf prinf_e prinf_d prinf_i
//  devel prerr
...
//  d<1> d<2> d<4> d<8> d<x> d<y> d<z>
//  c<1> c<2> c<4> c<8> c<x> c<y> c<z>
//
//  1 2 3 4 8 x y z p m
//  vec2 vec3 vec4
//  mat2 mat3 mat4
...
//  fix`array   arrfix      std.array<fix>    fixed_array!<int>      ::point !cond ? {10, 20} : {30, 40}
...
//  无符号类    字节    双字    四字    八字    十六    三二    六四    正数/字长   机器字长
//  有符号类    小整    短整    宽整    长整    大整    巨整    超整    整数        机器整数
//  定点类型    小定    短定  宽定/定点 长定    大定    巨定    超定
//  浮点类型    小浮    短浮  宽浮/浮点 长浮    大浮    巨浮    超浮
//  复数类型    小复    短复  宽复/复数 长复    大复    巨复    超复
//
//  泛参
//  具体类型 泛化类型 泛型正数 泛型整数 泛型浮点 泛型定点 泛型复数 泛型常数
...
//  array string slice
//
//  s01 s02 s04 s08 s16 s32 s64
//  b01 b02 b04 b08 b16 b32 b64
//  f01 f02 f04 f08 f16 f32 f64
//
//  1s  2s  4s  8s
//  1u  2u  4u  8u
//  1f  2f  4f  8f
...
//
//  vx`     // xmm i128 signed integer 16字节  128位
//  vy`     // ymm i256 signed integer 32字节  256位
//  vz`     // zmm i512 signed integer 64字节  512位
//
...
//  vx` float ffx f128    vx` decimal ddx d128    vx` complex ccx c128
...
//  def calc([const N]$T a) { prinf("size % % type %", typeof N, N, T) }
...
//      如果要像 C 一样修改参数，可以手动创建一个新的同名变量，覆盖参数的定义：
...
def calc(int a) { }
def calc(&int) { }
def calc(int a, &int float [x y] or error) { }
...
(point p, int a, &int) // 命名与类型同名的参数，不能写成 point point，两个类型名将触发返回值的声明的开始
(point p, int a, &int) // 简写形式，除了预定义类型参数类型名必须使用~前缀区分
(camera camera, point point)
(camera c, point p)
(point point) // 函数声明优先识别为类型，函数体内优先识别为变量名
(point p) (point p = expr) // 简写形式，使用 point` 避免与表达式语法冲突
(*camera camera, *point point)
(*point point)
(*camera camera, point point)
...
(int a, b, c, float d)
(int a, b = 2, c = 3, float d) // 允许第一个也使用点号，方便自动化工具
(int a, b, yield int, point, &int point)
(int a, b, yield int, point)
(int a, b, &int point float [count point scale])
...
//  1.  固定数组，是一个长度在编译时已经确定的数组，[3]int
//  2.  切片，是一个长度在运行时确定的数组，[:]int 。切片像是对固定数组的引用，它不存储任
//      何数据，只是对底层固定长度数组其中一段切片的描述。内部，切片仅仅保存一个数据指针
//      以及切片的长度。空切片 let a [:]int = null 长度为 0 并且不知想任何底层数组，切片
...
//  不保存到全局只读区。不保存大小，元素个数以及元素的大小都由编辑器记录。
//      def A = [_]int {1, 2, 3, 4, 5}
//      #ARRAY [5]int {1, 2, 3, 4, 5}
//      实现仅保存一个指向第一个元素的指针：
//      [1][2][3][4][5]
//      ^
//
//  固定数组，可写，不保存大小，元素个数以及元素的大小都由编辑器记录。
//      let a = [_]int {u, v, w, x, y, z}
//      $a [6]int {u, v, w, x, y, z}
//      实现仅保存一个指向第一个元素的指针，指针&1表示需要释放：
//      [u][v][w][x][y][z]    [alloc][u][v][w][x][y][z]
//      ^                            ^
//
//  原地数组，大小可变，但容量一旦分配后就不能改变，元素的大小由编译器记录。
//      let a = stack_array_fit(10, int) {a, b, c} // 分配在函数栈上
//      let a = array_fit(10, int) // 拥有10个元素容量的原地数组，使用 talloc 分配
//      [alloc][a][b][c][d][e][f]
//             ^
//      def array_fit $(anytype T) {
//          *T data
//          reg capacity
//          reg size
//      }
//
//  动态数组，其大小和容量都可以动态变化
//      let a = array(10, int) {a, b, c, d, e, f}
//      [alloc][a][b][c][d][e][f]
//             ^
//      def array_dyn $(anytype T) {
//          *T data
//          reg capacity
//          reg size
//      }
...
//
//  可写范围，不拥有内存
//      def range_mut $(anytype T) {
//          *T begin
//          *T end
//      }
//
//  数组切片，可写，不拥有内存，相当于不拥有内存的原地数组，可以不实现 slice，直接操作
//  原类型 array_fix array_fit array array_bdi 即可。
//      def array_slice $(anytype T) {
//          [*]T data // 指针的&1被剥离掉
//          reg size
//          reg start
//          reg capacity
...
def point = [3]float // 使用另一个类型定义一个新的类型
pub point = [3]float
...
def transfer this { // reader writer updater sender receiver transfer
...
// 个变量声明，在重载函数中可能会产生副作用。而 C# 就没有这么纠结这个问题，无论如何一
// 个二元运算表达式都不能单独形成独立的语句，遇到 foo * x 的情况，一定是一个变量声明。
...
def eat(*lexer lexer, token token) {
    return lexer.pop()
}

def peek(*lexer lexer, token token) {
    return lexer.top()
}
...
def tcp_poll(*file, *socket, *poll_table, &poll) [m] "alignas 16" {
    def poll = undefined
...
def perform_tcpa_open_accept(*TcpSocket tcp, r32 txbuf_size, r32 rxbuf_size) {
...
let ptr = alloc(1024) or panic()
let data = data {this, a = 1, 2, b = 3} // 元组类型变量定义 data.a data.b data[2]
let data = read_tuple(''_, a) // 元组类型值的返回 data[1] data.a
...
static if expr { calc Calc null } else { extern calc Calc }
...
//
//  if [expr] label == item // 该形式不允许 if [expr] 语句嵌套，否则报错，该形式禁止书写大括号，因为仅仅是一个标签
//      ...
//  label == item
//      ...
//  label == else // 不简单使用 else 的原因是，当作为标签用作其他条件语句块中时，可能意外终止最近的一个if
//      ...
//
//  label == .value // 该形式仅仅是一个标签，不应用任何 if 语句的缩进规则
