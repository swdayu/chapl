...
CORO_API(upr) coro_asm_init(upr rsp);
...
CORO_API(void) coro_stack_crash(upr co_rsp, upr rsp)
{
...
CORO_API(corotpool_t) coroutine_init(upr pool_id, upr count)
{
    ...
    upr alloc = sizeof(cocontext_t) + count * sizeof(coroutine_t*);
...
CORO_API(coroutine_t*) coroutine_create(corotpool_t pool, coro_func_t f, upr stack_size, upr para)
{
    ...
    if (ctx->count < ctx->capacity) {
        upr alloc = upr_times_of_16(sizeof(coroutine_t) + stack_size);
        void *p = sys_malloc(alloc); // 对齐到 16 字节边界
        void **rsp = (void **)((upr)p + alloc);
...
#endif
        lang_assert((upr)rsp_aligned_to_16 % 16 == 0);
        lang_assert((upr)align_to_16_bytes % 16 == 0);
        return co;
    }
    return null;
}
...
CORO_API(bool) coroutine_yield_manual(corotpool_t pool, coroutine_t *wait, upr retp)
{
    cocontext_t *ctx = (cocontext_t *)pool;
